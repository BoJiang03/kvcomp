...
is_pre_rope = False
quant_model = QuantMode.VectorQuant
...
for v_quant_scale_rel in np.arange(0.01, STEP_END_MAP[quant_model], STEP_LEN_MAP[quant_model]).tolist():
    ...
    quant_cr, huffman_cr, fse_cr = v_standalone_cr_evaluation(config)
    logger.info(f"coqa accuracy: {accuracy}")
    logger.info(f"quant cr: {quant_cr}, huffman cr: {huffman_cr}")
    quant_scale_rels.append(v_quant_scale_rel)
    ...
    if accuracy < accuracy_threshold:
        logger.info(f"best k_quant_scale_rel: {v_quant_scale_rel}")
        logger.info(f"quant cr: {quant_cr}, huffman cr: {huffman_cr}")
        break

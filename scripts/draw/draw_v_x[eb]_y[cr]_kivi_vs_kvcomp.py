...
figures_path = "../final_figure/v_cr/"
...
def draw_two_lines_pic(x, y1, y2, title, save_path, label1="Line 1", label2="Line 2"):
    ...
    plt.yticks(fontproperties=founders_reg_prop, fontsize=16)
    plt.tight_layout() # Adjust layout to prevent labels overlapping
    plt.savefig(os.path.join(save_path, f"v_cr.pdf"))
    plt.close()
...
for ctx_len in context_lengths:
    ...
    kivi_quant_scale_rels = []
    kivi_results = []
    kvcomp_quant_scale_rels = []
    kvcomp_results = []
    for setting, result in rt:
        assert len(result) == 1
        if setting[1].k_quant_mode == QuantMode.ChannelQuant:
            kivi_quant_scale_rels.append(setting[1].v_quant_scale_rel)
            kivi_results.append(result[0]["v_quant_crs"][0])
        else:
            kvcomp_quant_scale_rels.append(setting[1].v_quant_scale_rel)
            kvcomp_results.append(crs_to_cr(result[0]["v_huffman_crs"]))
    save_path = os.path.join(figures_path, str(ctx_len))
    os.makedirs(save_path, exist_ok=True)
    assert len(kivi_quant_scale_rels) == len(kvcomp_quant_scale_rels)
    for i in range(len(kivi_quant_scale_rels)):
        assert kivi_quant_scale_rels[i] == kvcomp_quant_scale_rels[i]
    draw_two_lines_pic(kivi_quant_scale_rels, kivi_results, kvcomp_results, f"V Compression Ratio", save_path, label1="kivi", label2="kvcomp")
